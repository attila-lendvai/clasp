...
def previous_stage_artifact(relative_path):
    # TODO fixme
    return os.path.join('../stage-1/build/boehm/', relative_path))

def previous_stage_clasp():
    # TODO fixme
    return previous_stage_artifact('clasp-boehm')
...
def build(bld):
    ...
    bld.clasp_executable = bld.path.find_or_declare(variant.executable_name(stage='i'))
    ...
    intrinsics_bitcode = previous_stage_artifact(variant.inline_bitcode_archive_name("intrinsics"))
    builtins_bitcode   = previous_stage_artifact(variant.inline_bitcode_archive_name("builtins"))
    fastgf_bitcode     = previous_stage_artifact(variant.inline_bitcode_archive_name("fastgf"))
    ...
    if (bld.stage_val <= -1):
        ...
        task.set_inputs([previous_stage_clasp(),
                         intrinsics_bitcode,
                         builtins_bitcode] +
    ...
    if (bld.stage_val >= 1):
        ...
        task.set_inputs([fastgf_bitcode,
                         builtins_bitcode,
                         intrinsics_bitcode] +
        ...
        make_link_executable_task(bld,
                                  bld.clasp_executable,
                                  aclasp_common_lisp_output_name_list,
                                  previous_stage_artifact(variant.cxx_all_bitcode_name()))
...
def make_link_executable_task(bld, executable_node, lisp_file_list, bitcode_node):
    ...
    if (bld.env.LTO_FLAG and bld.env['DEST_OS'] == DARWIN_OS):
        clasp_lto_o = bld.path.find_or_declare('%s_exec.lto.o' % variant.executable_name(stage = 'b'))
